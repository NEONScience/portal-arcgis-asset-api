...
const DOWNLOADS_PATH = path.join(dirName, 'downloads');
...
const geojsonToSites = (geojson = {}, getProperties = p => p) => {
  const sites = {};
  if (!geojson.features) { 
    console.log(chalk.red(`- - No features identified`));
    return sites; 
  }
  geojson.features.forEach((feature) => {
    if (!feature.geometry) { 
      console.log(chalk.red(`- - No geometry identified`));
      return;
    }
    const geometry = {
    ...
    if (!siteCode) { 
      console.log(chalk.red(`- - No site code found for feature properties`));
      return;
    }
    if (!sites[siteCode]) {
